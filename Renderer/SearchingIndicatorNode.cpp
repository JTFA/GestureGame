...
SearchingIndicatorNode::SearchingIndicatorNode(const char *id, Vector3 translation, Quaternion rotation, Vector3 scale):Node(id),
...
SearchingIndicatorNode* SearchingIndicatorNode::create(const char* id, Vector3 translation, Quaternion rotation, Vector3 scale)
{
    SearchingIndicatorNode* node = new SearchingIndicatorNode(id, translation, rotation, scale);
    return node;
}

void SearchingIndicatorNode::addQuadModelAndNode(float size)
{
...
Node* SearchingIndicatorNode::makeNodeQuadModel(float size)
{
...
Material* SearchingIndicatorNode::setTextureUnlitMaterial(Model* model, const char* texturePath, bool mipmap, bool isPoint)
{
    Material* material;
    if (isPoint)
    {
...
void SearchingIndicatorNode::searching(Vector3 translation, Quaternion rotation, Vector3 scale, float elapsedTime)
{
    if (_state != SEARCHING)
    {
        _state = SEARCHING;
...
float SearchingIndicatorNode::getAnimateProgress(float elapsedTime, float duration)
{
    _elapsedTime += elapsedTime;
...
void SearchingIndicatorNode::foundTrackable(Vector3 translation, Quaternion rotation, Vector3 scale, float elapsedTime)
{
    if (_state == SEARCHING)
    {
        _state = FINDING;
